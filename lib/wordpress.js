...
function normalizePostArticle(post) {
  ...
  // Extract named author from _embedded (only available when ?_embed is used)
  const wpAuthor = post._embedded?.author?.[0];
  const author = wpAuthor?.name
    ? {
        name: decodeHtml(wpAuthor.name),
        url: wpAuthor.link || null,
        slug: wpAuthor.slug || null,
      }
    : null;
...
export function stripForMetaDescription(html, maxLen = 160) {
  ...
  // Backtrack to the last word boundary; refuse to cut more than half the
  // budget so we never produce a tiny stub (e.g. "Read…").
  const safe = lastSpace >= Math.floor(maxLen / 2) ? sliced.slice(0, lastSpace) : sliced;
